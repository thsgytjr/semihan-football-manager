-- JindoFC upcoming_matches 테이블을 snake_case 스키마로 마이그레이션
...
BEGIN
  ...
  IF table_exists AND has_camel_case THEN
    ...
    SELECT 
      id,
      COALESCE(room_id, 'jindofc-lite-room-1'),
      "dateISO",
      location,
      ...
      -- captainIds도 JSONB에서 array로 변환
      CASE 
        WHEN "captainIds" IS NOT NULL AND jsonb_typeof("captainIds") = 'array' THEN 
          ARRAY(SELECT jsonb_array_elements_text("captainIds"))::uuid[]
        ELSE '{}'::uuid[]
      END as captain_ids,
      formations,
      COALESCE("teamCount", 2),
      COALESCE("isDraftMode", false),
      COALESCE("isDraftComplete", false),
...
DROP TRIGGER IF EXISTS trg_upcoming_matches_updated_at ON upcoming_matches;
CREATE TRIGGER trg_upcoming_matches_updated_at 
  BEFORE UPDATE ON upcoming_matches
  FOR EACH ROW 
  EXECUTE FUNCTION update_updated_at_column();

-- Step 7: RLS 정책 (필요한 경우)
...
DO $$
BEGIN
  RAISE NOTICE '✅ JindoFC upcoming_matches 테이블이 snake_case 스키마로 마이그레이션되었습니다.';
  RAISE NOTICE '📊 Mission FC, Semihan, DKSC와 동일한 스키마를 사용합니다.';
END $$;
