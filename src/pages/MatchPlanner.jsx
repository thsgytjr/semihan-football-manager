...
export default function MatchPlanner({players,matches,onSaveMatch,onDeleteMatch,onUpdateMatch,isAdmin}){
  ...
  return(<div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_600px]">
    <Card title="매치 설정"><div className="grid gap-4">
      ...
      <Row label="장소"><div className="grid gap-2">
        <select className="w-full rounded border border-gray-300 bg-white px-3 py-2 text-sm" value={locationPreset} onChange={e=>{const v=e.target.value;setLocationPreset(v);if(v==='coppell-west'){setLocationName('Coppell Middle School - West');setLocationAddress('2701 Ranch Trail, Coppell, TX 75019')}else if(v==='indoor-soccer-zone'){setLocationName('Indoor Soccer Zone');setLocationAddress('2323 Crown Rd, Dallas, TX 75229')}else{setLocationName('');setLocationAddress('')}}}>
          <option value="coppell-west">Coppell Middle School - West</option><option value="indoor-soccer-zone">Indoor Soccer Zone</option><option value="other">Other (Freeform)</option>
        </select>
      ...
      <Row label="팀 수"><div className="flex items-center gap-3">
      ...
      <Row label={<span className="flex items-center gap-2">참석 ({attendeeIds.length}명)<button type="button" onClick={toggleSelectAll} className="ml-2 rounded border border-gray-300 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-100">{allSelected?'모두 해제':'모두 선택'}</button></span>}>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3">
          {players.map(p=>{const mem=String(p.membership||'').trim(),isMember=(mem==='member'||mem.includes('정회원'));return(
            <label key={p.id} className={`flex items-center gap-2 rounded border px-3 py-2 ${attendeeIds.includes(p.id)?'border-emerald-400 bg-emerald-50':'border-gray-200 bg-white hover:bg-gray-50'}`}>
              <input type="checkbox" checked={attendeeIds.includes(p.id)} onChange={()=>toggle(p.id)}/><InitialAvatar id={p.id} name={p.name} size={24}/>
              <span className="text-sm flex-1 whitespace-normal break-words">{p.name}{(p.position||p.pos)==='GK'&&<em className="ml-1 text-xs text-gray-400">(GK)</em>}</span>
              {!isMember&&<GuestBadge/>}{isAdmin&&!hideOVR&&(p.position||p.pos)!=='GK'&&<span className="text-xs text-gray-500 shrink-0">OVR {p.ovr??overall(p)}</span>}
            </label>)})}
        </div>
      </Row>
      <div className="flex flex-wrap gap-2">{isAdmin&&(<button onClick={save} className="rounded bg-emerald-500 px-4 py-2 text-white font-semibold">매치 저장</button>)}</div>
    </div></Card>
    ...
    <div className="grid gap-4">
      <Card title="팀 배정 미리보기 (드래그 & 드랍 커스텀 가능)" right={<div className="hidden sm:flex items-center gap-2 text-xs text-gray-500">기준: {posAware?'포지션 분산':criterion} · <span className="font-medium">GK 평균 제외</span></div>}>
        ...
        <div className="mb-2 flex items-center justify-end text-[11px] text-gray-500">표기: <span className="ml-1 inline-flex items-center gap-1"><GuestBadge/> 게스트</span></div>
        <DndContext sensors={sensors} collisionDetection={pointerWithin} onDragStart={onDragStartHandler} onDragCancel={onDragCancel} onDragOver={onDragOverHandler} onDragEnd={onDragEndHandler}>
          <div className="grid gap-4" style={{gridTemplateColumns:"repeat(auto-fit, minmax(260px, 1fr))"}}>
            {previewTeams.map((list,i)=>(<div key={i} className="space-y-2"><TeamColumn teamIndex={i} labelKit={kitForTeam(i)} players={list} showOVR={isAdmin&&!hideOVR} isAdmin={isAdmin} dropHint={dropHint}/></div>))}
          </div>
          <DragOverlay>{activePlayerId?(<DragGhost player={players.find(p=>String(p.id)===String(activePlayerId))} showOVR={isAdmin&&!hideOVR}/>):null}</DragOverlay>
        </DndContext>
      </Card>

      <Card title="저장된 매치" right={<div className="text-xs text-gray-500"><span className="font-medium">GK 평균 제외</span></div>}>
        <SavedMatchesList matches={matches} players={players} isAdmin={isAdmin} enableLoadToPlanner={true} onLoadToPlanner={loadSavedIntoPlanner} onDeleteMatch={onDeleteMatch} onUpdateMatch={onUpdateMatch} showTeamOVRForAdmin={true} hideOVR={hideOVR}/>
      </Card>
    </div>

    {editorOpen&&(<FullscreenModal onClose={closeEditor}>
      ...
      <div className="mb-2 text-xs text-gray-500">컨텍스트: <b>저장본 편집</b> · 팀: <b>{editingTeamIdx+1}</b> · 포메이션: <b>{formations[editingTeamIdx]||'4-3-3'}</b> · 인원: <b>{(editorPlayers[editingTeamIdx]||[]).length}명</b></div>
...
function TeamColumn({teamIndex,labelKit,players,showOVR,isAdmin,dropHint}){const id=`team-${teamIndex}`,{setNodeRef,isOver}=useDroppable({id}),non=players.filter(p=>(p.position||p.pos)!=='GK'),sum=non.reduce((a,p)=>a+(p.ovr??overall(p)),0),avg=non.length?Math.round(sum/non.length):0,showIndicator=dropHint?.team===teamIndex,indicator=(<li className="my-1 h-2 rounded bg-emerald-500/70 animate-pulse shadow-[0_0_0_2px_rgba(16,185,129,.35)]"/>);const rendered=[];for(let i=0;i<players.length;i++){if(showIndicator&&dropHint.index===i)rendered.push(<React.Fragment key={`hint-${i}`}>{indicator}</React.Fragment>);rendered.push(<PlayerRow key={players[i].id} player={players[i]} showOVR={showOVR}/>)}if(showIndicator&&dropHint.index===players.length)rendered.push(<React.Fragment key="hint-end">{indicator}</React.Fragment>)
  return(<div ref={setNodeRef} className={`rounded-lg border bg-white transition ${isOver?'border-emerald-500 ring-2 ring-emerald-200':'border-gray-200'}`}>
    <div className={`mb-1 flex items-center justify-between px-3 py-2 text-xs ${labelKit.headerClass}`}><div className="font-semibold">팀 {teamIndex+1}</div><div className="opacity-80 flex items-center gap-2"><span>{labelKit.label} · {players.length}명</span>{isAdmin&&(<span className="hidden sm:inline">· <b>팀파워</b> {sum} · 평균 {avg}</span>)}</div></div>
...
function PlayerRow({player,showOVR}){const{attributes,listeners,setNodeRef,transform,transition,isDragging}=useSortable({id:String(player.id)}),style={transform:CSS.Transform.toString(transform),transition,opacity:isDragging?0.7:1,boxShadow:isDragging?'0 6px 18px rgba(0,0,0,.12)':undefined,borderRadius:8,background:isDragging?'rgba(16,185,129,0.06)':undefined},pos=positionGroupOf(player),isGK=pos==='GK',ovrVal=player.ovr??overall(player);return(<li ref={setNodeRef} style={style} className="flex items-start gap-2 border-t border-gray-100 pt-1 first:border-0 first:pt-0 touch-manipulation cursor-grab active:cursor-grabbing" {...attributes}{...listeners}>
  <span className="flex items-center gap-2 min-w-0 flex-1"><InitialAvatar id={player.id} name={player.name} size={24}/><span className="whitespace-normal break-words">{player.name}</span>
  ...
  </span>{showOVR&&!isGK&&(<span className="ovr-chip shrink-0 rounded-full bg-stone-900 text-white text-[11px] px-2 py-[2px] tabular-nums">OVR {ovrVal}</span>)}
</li>)}
function DragGhost({player,showOVR}){if(!player)return null;const pos=positionGroupOf(player),isGK=pos==='GK';return(<div className="rounded-full border border-emerald-300 bg-white px-3 py-1.5 shadow-xl text-sm flex items-center gap-2 scale-[1.04]" style={{filter:'drop-shadow(0 8px 20px rgba(0,0,0,.18))'}}><span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-white text-[11px]">⇆</span><span className="font-medium">{player.name}</span><span className="text-[11px] rounded-full bg-stone-100 px-2 py-[2px]">{pos}</span>{showOVR&&!isGK&&<span className="text-gray-600 text-[12px]">OVR {player.ovr??overall(player)}</span>}</div>)}
function kitForTeam(i){const a=[{label:'화이트',headerClass:'bg-white text-stone-800 border-b border-stone-300'},{label:'블랙',headerClass:'bg-stone-900 text-white border-b border-stone-900'},{label:'블루',headerClass:'bg-blue-600 text-white border-b border-blue-700'},{label:'레드',headerClass:'bg-red-600 text-white border-b border-red-700'},{label:'그린',headerClass:'bg-emerald-600 text-white border-b border-emerald-700'},{label:'퍼플',headerClass:'bg-violet-600 text-white border-b border-violet-700'},{label:'오렌지',headerClass:'bg-orange-500 text-white border-b border-orange-600'},{label:'티얼',headerClass:'bg-teal-600 text-white border-b border-teal-700'},{label:'핑크',headerClass:'bg-pink-600 text-white border-b border-pink-700'},{label:'옐로',headerClass:'bg-yellow-400 text-stone-900 border-b border-yellow-500'},];return a[i%a.length]}
function FullscreenModal({children,onClose}){return(<div className="fixed inset-0 z-50 bg-black/40"><div className="absolute inset-4 md:inset-10 rounded-lg bg-white p-4 overflow-auto"><div className="mb-2 flex justify-end"><button onClick={onClose} className="rounded border border-gray-300 bg-white px-3 py-1 text-sm">닫기</button></div>{children}</div></div>)}
function GuestBadge(){return(<span className="inline-flex items-center justify-center rounded px-1.5 py-0.5 text-[10px] font-semibold text-rose-700 bg-rose-50 border border-rose-200">G</span>)}
