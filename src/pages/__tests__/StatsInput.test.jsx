...
describe('StatsInput - 기록 입력', () => {
  ...
  it('Bulk 입력이 잘못된 형식이면 오류 메시지를 노출한다', async () => {
    ...
    const textarea = screen.getByPlaceholderText(/예시:/i)
    fireEvent.change(textarea, { target: { value: '잘못된 형식' } })
    const applyButton = screen.getAllByRole('button', { name: '✨ 초안에 적용하기' })[0]
    fireEvent.click(applyButton)

    expect(await screen.findByText(/모든 줄이 .*형식이어야 합니다/)).toBeInTheDocument()
  })
  ...
  it('goal:assist Bulk 입력을 적용 후 저장하면 링크가 유지된 채 onUpdateMatch로 전달된다', async () => {
    ...
    const textarea = screen.getByPlaceholderText(/예시:/i)
    fireEvent.change(textarea, { target: { value: '[11/08/2025 9:07AM]goal:assist[Alpha Beta]' } })
    const applyButton = screen.getAllByRole('button', { name: '✨ 초안에 적용하기' })[0]
    fireEvent.click(applyButton)

    await screen.findByText(/초안에 적용 완료/i)
    ...
    expect(stats.p1.events[0].assistedBy).toBe('p2')
    expect(stats.p2.events[0].linkedToGoal).toBe('p1')
  })
