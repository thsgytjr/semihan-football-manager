...
export default function MiniPitch({ placed = [], height = 150, onEdit, formation, mode }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white">
      ...
      <div className="relative overflow-hidden rounded-md" style={{ height }}>
        ...
        <PitchLines />
        {nodes.map(p => (
          <div key={p.id} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${p.x}%`, top: `${p.y}%` }} title={`${p.name} (${p.role})`}>
